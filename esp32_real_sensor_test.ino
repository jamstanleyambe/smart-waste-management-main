...
// Try different IP addresses - your ESP32 might be on a different network
const char* serverUrls[] = {
  ...
  "http://10.0.0.100:8000/api/sensor-data/"        // Another common IP
};
...
void setup() {
  ...
  Serial.println("\n=== ESP32 REAL SENSOR DATA TEST ===");
  Serial.println("🎯 Goal: Send REAL sensor data to Django server");
  ...
  Serial.println("🔍 Testing different server URLs...");
  Serial.println();
}
...
float readSensor() {
  Serial.println("┌────────── REAL SENSOR READING ─────────┐");
...
void sendData() {
  ...
  const char* currentUrl = serverUrls[currentServerIndex];
  ...
  // Create JSON payload with REAL sensor data
  StaticJsonDocument<200> doc;
  doc["sensor_id"] = "ESP32_001";
  doc["bin_id"] = binId;
  doc["fill_level"] = currentFillPercentage;  // REAL sensor reading
  ...
  if (httpCode > 0) {
    ...
    if (httpCode == 201) {
      Serial.println("- ✅ SUCCESS! Real sensor data sent!");
      ...
      Serial.println("🎉 REAL SENSOR DATA SUCCESSFULLY SENT TO SERVER!");
      Serial.println("📊 Check your dashboard - BIN001 should now show REAL data!");
...
void tryNextServer() {
  currentServerIndex = (currentServerIndex + 1) % 4;
